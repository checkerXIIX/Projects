...
def remove_stopwords(data):
    ...
    for w in word_tokens:
        if w not in stop_words:
            filtered_sentence += ' ' + w # only add words that aren't stopwords

    filtered_sentence = ' '.join(filtered_sentence.split()) # remove space from the beginning
...
def create_sequences(sequence_data, sequence_len, vocab_size, visualization_status):
    sequences = []
    for i in range(sequence_len, len(sequence_data)): # loop starts from 5 (sequnence length) -> sequence is always full
        words = sequence_data[i - sequence_len:i + 1]  # always 6 words (5 input, 1 prediction)
    ...
    y = []
    for i in sequences:
        X.append(i[0:sequence_len])  # input are the last 5 words
        y.append(i[sequence_len])  # prediction should be the next word
    ...
    if visualization_status == False: # if prediction should be visualised -> don't one hot encode
...
def trainings_pipeline():
    ...
    sequence_len = 10   # should be as big as possible
    ...
    #Models with varying embedding size
    model1 = build_model_bidirectional_lstm(nodes, 100, sequence_len, vocab_size)
    model2 = build_model_double_lstm(nodes, 250, sequence_len, vocab_size)
    model3 = build_model_bidirectional_lstm(nodes, 500, sequence_len, vocab_size)

    batch_size = 64
    # models trained with varying number of epochs
