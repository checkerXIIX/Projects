...
void auctionRound_parallel(int id, vector<vector<int>> cost, int n, stack<int>& unassigned, vector<int>& assignment, vector<int>& belong, vector<double>& price, double epsilon);

mutex mt_; // init mutex

int auction_parallel(vector<vector<int>> &cost, int n)
{
	vector<int> assignment(n, -1); //vector that safes the task j that resource i is assigned to
	vector<int> belong(n, -1); // vector that safes the resource that task j is assigned to
	vector<double> prices(n, 1); // prices of tasks
	double epsilon = 1.0;
	stack<int> unassigned; // stack to store unassigned resources

	// Epsilon-scaling
	while(epsilon > 1.0/n)
	{
		// Create threads for auction round
		...
		for (int i=0; i < NUM_THREADS; i++){
		   threads.emplace_back(auctionRound_parallel, i, cost, n, ref(unassigned), ref(assignment), ref(belong), ref(prices), epsilon);
		}
...
void auctionRound_parallel(int id, vector<vector<int>> cost, int n, stack<int>& unassigned, vector<int>& assignment, vector<int>& belong, vector<double>& prices, double epsilon)
{
	/* Compute the bids of each unassigned individual and assign them */
	while (true)
	{
		mt_.lock();
		...
		vector<double> prices_thread(prices);
        mt_.unlock();
		...
		*/
		...
		for (int j = 0; j < n; j++)
		{
			double curVal = cost[bidder][j] - prices_thread[j];
			if (curVal > best_payoff)
			{
				sec_payoff = best_payoff;
				best_payoff = curVal;
				best_task = j;
				price_task = prices_thread[j];
			}
...
int main(int argc, char*argv[])
{
	...
	out_file_all<<"Auction Parallel Manual ("<<NUM_THREADS<<" Threads): \n";
	out_file_all<<"Size\t\t\t\tCost\t\t\t\tTotal Time in s\t\t\tTotal CPU time in ms\n";
	
	ofstream out_file_mean;
    out_file_mean.open (filename_mean_runs);
	out_file_mean<<"Auction Parallel Manual ("<<NUM_THREADS<<" Threads, "<<num_test_runs<<" runs): \n";
