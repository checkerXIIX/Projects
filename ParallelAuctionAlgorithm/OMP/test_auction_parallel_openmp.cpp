...
void auctionRound_parallel(vector<vector<int>> &cost, int n, vector<int>& assignment, vector<int>& belong, vector<double>& prices, double epsilon);
...
int auction_parallel(vector<vector<int>> &cost, int n)
{
	...
	while(epsilon > 1.0/n)
	{
		...
		{
			auctionRound_parallel(cost, n, assignment, belong, prices, epsilon);
		}
...
void auctionRound_parallel(vector<vector<int>> &cost, int n, vector<int>& assignment, vector<int>& belong, vector<double>& prices, double epsilon)
{
	...
	#pragma omp declare reduction (merge : std::vector<bid> : omp_out.insert(omp_out.end(), omp_in.begin(), omp_in.end()))
...
int main(int argc, char*argv[])
{
	...
	out_file_all<<"Auction Parallel OpenMP ("<<NUM_THREADS<<" Threads): \n";
	out_file_all<<"Size\t\t\t\tCost\t\t\t\tTotal Time in s\t\t\tTotal CPU time in ms\n";
	
	ofstream out_file_mean;
    out_file_mean.open (filename_mean_runs);
	out_file_mean<<"Auction Parallel OpenMP ("<<NUM_THREADS<<" Threads, "<<num_test_runs<<" runs): \n";
